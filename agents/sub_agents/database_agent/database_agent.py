# Database Agent with LangGraph Implementation
...
# database system prompt. If the question is related to databases,
# then the agent responds with a database based dummy valid response and
# if the question is not about databases, then the agent provides an error
...
# This is the dummy response that the database agent provides if the question is about database management
valid_response_fpath: str = os.path.abspath(config_data['agent_infra_resources'].get('valid_response_path'))
# This is the dummy response that the database agent provides if the question is not about databases, in this case
...
print(f"Going to use the dummy valid response path: {valid_response_fpath} and the dummy erroneous response path: {erroneous_response_fpath} to simulate the database agent's behaviour...")
...
print(f"Initialized the ChatBedrock model for the database agent: {model}")
...
def health_check() -> Dict:
    ...
    return {
        ...
        "response_message": "Health check passed - Database Agent is running",
    }
...
def extract_json_from_response(response_text):
    ...
    try:
        ...
        logger.debug(f"Failed to parse full response as JSON: {e}")
        ...
        json_patterns = [
            ...
            r'(\{[^{}]*"is_database"[^{}]*\})',  # Look for objects containing "is_database"
            r'(\{.*?\})'                    # Any JSON object
        ]
        ...
        for pattern in json_patterns:
            ...
            if match:
                try:
                    ...
                    logger.debug(f"Failed to parse extracted JSON: {nested_e}")
                    continue
...
class AgentState(TypedDict):
    """State for the Database agent graph."""
    user_question: str
...
def build_database_agent_graph():
...
@app.entrypoint
def database_agent_handler(payload: Dict) -> Dict:
    """Entrypoint for the Database agent using custom LangGraph nodes."""
    # Build the custom agent graph
    agent_graph = build_database_agent_graph()
    ...
    try:
        ...
        logger.error(f"Error in database_agent_handler: {e}")
        return {
            "statusCode": 500,
            "response_type": ResponseType.ERROR,
...
def interactive_cli():
    ...
    print("\n🤖 Database Agent - LangGraph with Custom Nodes and Response Types")
