# Development Tools Agent with LangGraph Implementation
...
# development tools system prompt. If the question is related to development tools,
# then the agent responds with a development tools based dummy valid response and
# if the question is not about development tools, then the agent provides an error
...
# This is the dummy response that the development tools agent provides if the question is about development tools
valid_response_fpath: str = os.path.abspath(config_data['agent_infra_resources'].get('valid_response_path'))
# This is the dummy response that the development tools agent provides if the question is not about development tools, in this case
...
print(f"Going to use the dummy valid response path: {valid_response_fpath} and the dummy erroneous response path: {erroneous_response_fpath} to simulate the development tools agent's behaviour...")
...
try:
    ...
    agent_prompt_template_content = agent_prompt_template_raw.format(
        ...
        model_response=ResponseType.MORE_INFO_NEEDED,
...
# Read valid and erroneous dummy content that needs to be provided back to the orchestrator agent
try:
    with open(valid_response_fpath, 'r') as f:
        valid_content = json.loads(f.read())
    with open(erroneous_response_fpath, 'r') as f:
        error_content = json.loads(f.read())
except (FileNotFoundError, json.JSONDecodeError) as e:
    print(f"Error reading response files: {e}")
    # Use fallback content if files don't exist
    valid_content = {
        "response_type": ResponseType.MORE_INFO_NEEDED,
        "response_message": "This is a valid Development Tools-related response.",
        "status": "success"
    }
    error_content = {
        "response_type": ResponseType.OUT_OF_SCOPE,
        "response_message": "This question is not related to Development Tools. Please ask about Development Tools-specific topics.",
        "status": "error"
    }

print(f"Going to use the dummy valid content information: {valid_content}")
print(f"Going to use the dummy error content: {error_content}")
...
print(f"Initialized the ChatBedrock model for the development tools agent: {model}")
...
def health_check() -> Dict:
    ...
    return {
        "statusCode": 200,
        "response_type": ResponseType.MORE_INFO_NEEDED,
        "response_message": "Health check passed - Development Tools Agent is running",
    }
...
def extract_json_from_response(response_text):
    ...
    try:
        ...
        json_patterns = [
            ...
            r'(\{[^{}]*"is_development_tools"[^{}]*\})',  # Look for objects containing "is_development_tools"
            r'(\{.*?\})'                    # Any JSON object
        ]
...
class AgentState(TypedDict):
    """State for the Development Tools agent graph."""
    user_question: str
...
def build_development_tools_agent_graph():
...
@app.entrypoint
def development_tools_agent_handler(payload: Dict) -> Dict:
    """Entrypoint for the Development Tools agent using custom LangGraph nodes."""
    # Build the custom agent graph
    agent_graph = build_development_tools_agent_graph()
    ...
    try:
        ...
        logger.error(f"Error in development_tools_agent_handler: {e}")
        return {
            "statusCode": 500,
            "response_type": ResponseType.ERROR,
...
def interactive_cli():
    ...
    print("\n🤖 Development Tools Agent - LangGraph with Custom Nodes and Response Types")
